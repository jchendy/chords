...
| `neck.js` | Draws a full 15-fret neck as SVG from markers and shape outlines — shared by the practice fretboard, the chord finder's CAGED overview and the reverse finder, so all three necks are one drawing. A fret is half again as wide as the gap between two strings, near enough the shape of the real thing to read a grip off, and a note sits behind its fret wire where the finger goes rather than in the middle of the gap; the inlays and the fret numbers stay centred, since that's where they are on a guitar. A full neck is wide, so it wants most of a laptop's width — hence the wider cap on how large the drawing may render. |
